...
create table usertbl(
   userid varchar(10) primary key,      -- 사용자 아이디(PK)
    name varchar(10) not null,         -- 사용자 이름
    birthyear int not null,            -- 사용자의 출생년도
    addr varchar(10) not null,         -- 사용자의 주소
    mobile1 varchar(3),               -- 휴대폰의 앞자리
    mobile2 varchar(8),               -- 휴대폰의 뒷자리
    height smallint,               -- 사용자의 키(smallint형은 2바이트)
    mdate date                     -- 회원 가입일
);
...
create table buytbl(
   -- auto_increment명령어는 mysql엔진이 데이터가 들어올 때마다, 1씩 자동증가 시켜줌
   num int auto_increment primary key,
    -- userid는 여기서는 pk가 될수 없다. 그 이유는 한 테이블에는 pk는 오로지 
    -- 하나만 존재해야 한다.
    userid varchar(10) not null,
    prodName varchar(10) not null,      -- 구매 물품명
    groupName varchar(10),            -- 물품 분류
    price int not null,               -- 물품 단가
    amount smallint not null,         -- 구매 수량
    -- usertbl에 있는 userid를 참조해라.여기서는 usertbl이 외래키로써(fk)의 역할을 한다.
